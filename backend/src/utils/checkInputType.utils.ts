...
export const isInputTypeUrl = (url: string) => {
    logger.info("utils/backend: check-input-type :: " + "EXECUTION START: isInputTypeUrl: " + url);
    let urlRequest = url;
    try {
        ...
        if (urlRequest.indexOf('.') <= 0 || urlRequest.indexOf(' ') > 0) {
            logger.info("utils/backend: check-input-type :: " + "EXECUTION END: isInputTypeUrl: " + "Input type is a tag: " + urlRequest);
            return null
        }
