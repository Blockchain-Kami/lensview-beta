...
export const preprocessURL = (url) => {
  logger.info(
    "utils/backend: process-url :: " + "EXECUTION START: preprocessURL: " + url
  );
  try {
    ...
    logger.info(
      "utils/backend: process-url :: " +
        "EXECUTION END: preprocessURL: Cleaned URL: " +
        cleanURL
    );
    return [
