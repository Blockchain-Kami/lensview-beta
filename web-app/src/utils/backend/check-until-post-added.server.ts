...
export const checkUntilMainPostAdded = async (txHash, startTime) => {
	...
	logger.info(
		'utils/backend: check-until-post-added :: ' + 'EXECUTION START: checkUntilMainPostAdded'
	);
	if (Date.now() - startTime > 25000) {
		logger.error(
			'utils/backend: check-until-post-added :: ' +
				'EXECUTION END: checkUntilMainPostAdded: FAILED: Error Indexing Transaction Hash: ' +
				txHash
		);
		return false;
	}
	...
	if (hasIndexedResponse?.data?.hasTxHashBeenIndexed?.indexed) {
		logger.info(
			'utils/backend: check-until-post-added :: ' +
				'EXECUTION END: checkUntilMainPostAdded: SUCCESS: Indexed Transaction Hash: ' +
				txHash
		);
		return true;
	} else {
		logger.info(
			'utils/backend: check-until-post-added :: ' +
				'Indexing: checkUntilMainPostAdded: Waiting to Index Transaction Hash: ' +
				txHash
		);
