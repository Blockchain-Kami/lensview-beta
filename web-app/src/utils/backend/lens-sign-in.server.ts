...
export const signInWithLens = async () => {
  logger.info(
    "utils/backend: lens-sign-in :: " + "EXECUTION START: signInWithLens"
  );
  try {
    // console.log(challenge);
    /* first request the challenge from the API server */
    const challengeInfo = await client
      .query(challenge, { address: APP_ADDRESS })
      .toPromise();
    ...
    const authData = await client
      .mutation(authenticate, { address: APP_ADDRESS, signature: signature })
      .toPromise();
    ...
    if (profile != null) {
      logger.info(
        "utils/backend: lens-sign-in :: " +
          "EXECUTION END: signInWithLens: " +
          "SUCCESSFUL"
      );
...
const getUserProfile = async () => {
  logger.info(
    "utils/backend: lens-sign-in :: " + "EXECUTION START: getUserProfile"
  );
  try {
    ...
    logger.info(
      "utils/backend: lens-sign-in :: " +
        "EXECUTION END: getUserProfile: " +
        "SUCCESSFUL: Signed in with: " +
        defaultProfile
    );
