...
const commentAnonymously = async (pubId, content, client, signer, profile) => {
	logger.info(
		'utils/backend: comment-anonymously.server.ts :: ' +
			'EXECUTION START: commentAnonymously : pubID ' +
			pubId
	);
	isPosting = true;
	...
	if (contentURI === null) {
		logger.error(
			'utils/backend: comment-anonymously.server.ts :: ' +
				'EXECUTION END: commentAnonymously: Failed'
		);
	}
	...
	try {
		...
		await tx.wait();
		logger.info(
			'utils/backend: comment-anonymously.server.ts :: ' + 'Transaction Sent: commentAnonymously'
		);
		isPosting = false;
		logger.info(
			'utils/backend: comment-anonymously.server.ts :: ' +
				'Transaction Confirmed: commentAnonymously : Transaction Hash: ' +
				tx.hash
		);
		logger.info(
			'utils/backend: comment-anonymously.server.ts :: ' +
				'EXECUTION END: commentAnonymously : Commented Anonymously on pubID: ' +
				pubId
		);
		return tx.hash;
	} catch (error) {
		logger.info(
			'utils/backend: comment-anonymously.server.ts :: ' +
				'EXECUTION END: commentAnonymously : Failed to add anonymous comment to pubID: ' +
				pubId
		);
		isPosting = false;
		return isPosting;
