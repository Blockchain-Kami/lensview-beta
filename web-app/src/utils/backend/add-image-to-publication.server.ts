...
export const addImageToPublication = async (job) => {
    ...
    logger.info("utils/backend : add-image-to-publication.server.ts:: " + "EXECUTION START: addImageToPublication: " + "Adding Image for URL: " + urlObj.url);
    ...
    const response = await addImageComment(urlObj, parentPostID , client, signer, profile);
    if (response) {
        logger.info("utils/backend : add-image-to-publication.server.ts:: " + "EXECUTION END: addImageToPublication: " + "DONE: Image Added for URL " + urlObj.url);
        return
    } else {
        logger.error("utils/backend : add-image-to-publication.server.ts:: " + "EXECUTION END: addImageToPublication: " + "FAILED: Image Not Added for URL " + urlObj.url);
        return
    }
}