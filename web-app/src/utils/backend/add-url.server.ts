...
const savePost = async (urlObj, client, signer, profile) => {
	...
	try {
		...
		const tx = await contract.postWithSig(
			{
				...
				collectModuleInitData:
					signedResult.result.data.createPostTypedData.typedData.value.collectModuleInitData,
				referenceModule:
					signedResult.result.data.createPostTypedData.typedData.value.referenceModule,
				referenceModuleInitData:
					signedResult.result.data.createPostTypedData.typedData.value.referenceModuleInitData,
				sig: {
					v,
					r,
					s,
		...
		isPosting = false;
		logger.info(
			'utils/backend: add-url.server.ts :: ' +
				'Transaction Confirmed: savePost : Transaction Hash: ' +
				tx.hash
		);
		logger.info(
			'utils/backend: add-url.server.ts :: ' +
				'EXECUTION END: savePost : URL added successfully: ' +
				urlObj.url
		);
		return tx.hash;
	} catch (error) {
		logger.info(
			'utils/backend: add-url.server.ts :: ' +
				'EXECUTION END: savePost : Failed to add URL: ' +
				urlObj.url
		);
		isPosting = false;
		return isPosting;
