...
const savePost = async (urlObj, client, signer, profile) => {
  logger.info(
    "utils/backend: add-url.server.ts :: " + "EXECUTION START: savePost"
  );
  isPosting = true;
  ...
  try {
    ...
    const tx = await contract.postWithSig(
      {
        profileId:
          signedResult.result.data.createPostTypedData.typedData.value
            .profileId,
        contentURI:
          signedResult.result.data.createPostTypedData.typedData.value
            .contentURI,
    ...
    await tx.wait();
    logger.info(
      "utils/backend: add-url.server.ts :: " + "Transaction Sent: savePost"
    );
    isPosting = false;
    logger.info(
