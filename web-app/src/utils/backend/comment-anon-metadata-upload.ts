...
function makeFileObjects(content) {
	...
	logger.info(
		'utils/backend: comment-anon-metadata.server.ts :: ' + 'EXECUTION START: makeFileObjects'
	);
	const metaData = {
		version: '2.0.0',
		content: content,
	...
	try {
		logger.info(
			'utils/backend: comment-anon-metadata.server.ts :: ' +
...
const uploadCommentToIPFS = async (content) => {
	logger.info(
		'utils/backend: comment-anon-metadata.server.ts :: ' + 'EXECUTION START: uploadCommentToIPFS.'
	);
	...
	try {
		...
		if (files === null) {
			logger.error(
				'utils/backend: comment-anon-metadata.server.ts :: ' +
					'EXECUTION END: uploadCommentToIPFS: Failed'
			);
			return null;
		}
		...
		logger.info(
			'utils/backend: comment-anon-metadata.server.ts :: ' +
				'EXECUTION END: uploadCommentToIPFS : Stored file to IPFS, URI:  ' +
				uri
		);
		return uri;
	} catch (error) {
		logger.error(
			'utils/backend: comment-anon-metadata.server.ts :: ' +
				'EXECUTION END: uploadCommentToIPFS : Failed to upload metadata to IPFS: ' +
				error
		);
		return null;
	}
