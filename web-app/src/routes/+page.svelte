...
<MediaQuery query="(max-width: 825px)" let:matches>
	<section>
										...
										<div class="CenterRowFlex card__post__info__head">
											...
											<div class="CenterRowFlex card__post__info__head__trend">
												...
												<div class="card__post__info__head__trend__count">
													{data?.data?.publications?.items[0]?.stats?.totalUpvotes === undefined
														? 0
														: data?.data?.publications?.items[0]?.stats?.totalUpvotes}
												</div>
											</div>
		...
		<button on:click={() => (showAddNewPostModal = true)} class="CenterRowFlex add__post">
