<script lang="ts">
	...
	const redirectToPostsOrSearchPage = async () => {
		...
		try {
			const fetchedInfoForSearchedInput: FetchedInfoForSearchedInputModel = await fetch(
				'/api/is-url-valid-get-parent-pubId',
				{
					method: 'POST',
					headers: {
...
<Notifications item={CustomNotification} zIndex="20">
	<div class="CenterRowFlex nav">
		...
		<form on:submit|preventDefault={() => redirectToPostsOrSearchPage()} class="nav__search">
			<input
			...
			>
				...
				{:else if !isConnected}
					<div class="menu__connect-box">
						<div class="menu__connect-box__text">Hello friend! Welcome to LensView.</div>
					...
					<div class="menu__connect-box">
						...
						<div class="menu__connect-box__btn">
							{#if !signingIn}
								...
								{:else}
									<button on:click={() => (showCreateLensHandleModal = true)} class="btn"
										>Create Lens Handle
									</button>
								{/if}
							{:else}
