...
export async function POST(requestEvent) {
	logger.info("routes/api/comment-anonymously: +server.ts :: " + "EXECUTION START");
	const { request } = requestEvent;
	const requestBody = await request.json();

	const pubId = requestBody['pubId'];
	logger.info("routes/api/comment-anonymously: +server.ts :: " + "Commenting on Publication: " + pubId);
	...
	if (txHash) {
		logger.info("routes/api/comment-anonymously: +server.ts :: " + "EXECUTION END: DONE: Anonymous Comment Added to Publication: " + pubId);
		return json({
			message: `Anonymous comment added to publication ID: ${pubId}`
		});
	} else {
		logger.info("routes/api/comment-anonymously: +server.ts :: " + "EXECUTION END: FAILED: Could Not Comment Anonymously on Publication: " + pubId);
		throw error(500, {
