	...
	const postAnonymously = async () => {
		...
		try {
			...
			await fetch('/api/comment-anonymously', {
				...
				body: JSON.stringify({
					pubId:
						$page.data.postPubId !== undefined ? $page.data.postPubId : $page.data.mainPostPubId,
			...
			addNotification({
				...
				ctaFunction: () => {
					if (mainPostPubId === $page.data.mainPostPubId && postPubId === $page.data.postPubId) {
...
<section>
	<div class="CenterRowFlex body">
		<div class="body__user-pic">
			{#if $isSignedIn}
				<img
					src={getPictureURL($userProfile?.picture?.original?.url, $userProfile?.ownedBy)}
					alt=""
				/>
			{:else}
